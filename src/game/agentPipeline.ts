...
export async function planContent(
  ...
  const prompt = [
    `너는 "마이야르" 요리 레시피 SEO 콘텐츠 회사의 "${opts.deptName}" 팀장 ${opts.leadName}이야.`,
    `오늘 발행할 콘텐츠 기획안을 스스로 정해서 대표에게 보고해야 해.`,
    `"고기 굽는 방법/온도/부위별 가이드" 같은 대중적인 검색 수요가 있는 요리 주제를 골라줘.`,
    avoidList,
    instructionLine,
    ...
    `  "reason": "대표에게 보고할 한두 문장 — 왜 오늘 이 주제를 선택했는지"`,
    `}`,
  ].join("\n");
...
export async function writeDraft(
  ...
  const prompt = [
    `너는 "마이야르" 요리 레시피 SEO 콘텐츠 회사의 원고 작성 담당 ${opts.leadName}이야.`,
...
export async function reviewDraft(
  apiKey: string,
  markdown: string,
  opts: { leadName: string; keyword: string },
): Promise<ReviewResult> {
  const prompt = [
    `너는 "마이야르" 요리 레시피 SEO 콘텐츠 회사의 검수 담당 ${opts.leadName}이야.`,
    `아래 원고가 실제로 발행해도 될 만큼 품질이 충분한지 깐깐하게 평가해줘.`,
    `타깃 키워드 "${opts.keyword}"가 자연스럽게 들어갔는지, 온도·시간 등 구체적 수치가 있는지,`,
    `과장 표현은 없는지, 구성(재료→조리법→팁→FAQ→보관법)이 되어 있는지 확인해줘.`,
    ``,
    `[원고]`,
