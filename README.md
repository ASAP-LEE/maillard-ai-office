...
1. **키워드 기획 팀장**이 오늘 어떤 콘텐츠를 만들지 스스로 판단해서 보고해요.
   회사가 실제로 다루는 3가지 카테고리 중 하나를 매번 골라요:
   - **온도·시간 가이드** — 부위별 굽는 온도, 시간, 굽기 정도
   - **고기 소개** — 특정 부위/품종(등심, 안심, 삼겹살, 와규, 한우 등급 등)이 뭐가 특별한지
   - **맛 비교·랭킹** — 여러 부위/품종을 비교해서 뭐가 더 맛있는지, 어떤 상황에 뭐가 나은지
   (이미 발행된 글과 중복되지 않게 신경 써요)
2. 대표가 **승인**하면 → **원고 팀장**이 그 카테고리에 맞는 구성으로 실제 원고를 씀
3. 원고가 나오면 **검수 팀장**이 실제로 평가해서 통과/반려를 판단
   - 반려면 피드백을 받아서 원고 팀장이 다시 씀 (최대 2회 재시도)
   - 통과하면 원고가 확정되고 목록에 쌓여요 (개별/전체 **.md 다운로드** 가능)

**미승인을 누르면** 텍스트 입력창이 열리고, 거기에 "이 주제 말고 다른 부위로",
"고기 소개 말고 비교 콘텐츠로", "더 캐주얼한 톤으로" 같은 지시를 적어서 보내면,
기획 팀장이 그 지시를 반영해서 다시 보고해요.
